...
from schemas.pydantic.DocumentSchema import DocumentCreate, DocumentUpdate, DocumentResponse
from domain.storage import StoredFile, IStorageService, StorageStatus
...
def create_test_file(content: bytes = b"test content") -> UploadFile:
...
def storage_service():
    ...
    async def mock_store(file_data, file_id, user_id, mime_type):
    ...
    async def mock_retrieve(file_id, user_id, owner_id=None):
    ...
    mock_storage.get_metadata = AsyncMock(side_effect=mock_get_metadata)
    return mock_storage
...
class TestDocumentIntegration:
    ...
    async def test_create_and_retrieve_document(
        ...
        assert retrieved_doc.doc_metadata == created_doc.doc_metadata
    ...
    async def test_update_document(
        ...
        assert updated_doc.doc_metadata.get("description") == "Updated Description"
    ...
    async def test_delete_document(
        ...
        doc_create = DocumentCreate(
            name="To Be Deleted",
            metadata={"description": "This document will be deleted"},
            is_public=False,
            mime_type="text/plain",
        )
        test_file = create_test_file()
    ...
    async def test_public_document_access(
        ...
        doc_create = DocumentCreate(
            ...
            metadata={"description": "This is a public document"},
            is_public=True,
            mime_type="text/plain",
        )
        test_file = create_test_file()
    ...
    async def test_document_list_and_filter(
        ...
        for name in doc_names:
            ...
            test_file = create_test_file(content=f"Content for {name}".encode())
        ...
        assert len(documents) >= 3  # May include documents from other tests

        # Test filtering by name
        filtered_docs = await document_service.list_documents(
            user_id=sample_user.id,
            skip=0,
            limit=10,
            filters={"name": "Doc1"}
    ...
    async def test_document_size_limits(
        ...
        doc_create = DocumentCreate(
            ...
            metadata={"description": "This is a valid size document"},
            is_public=False,
        ...
        assert len(file_content.read()) == len(valid_content)

        # Test with an oversized document (11MB)
        oversized_content = b"x" * (11 * 1024 * 1024)  # 11MB
        doc_create = DocumentCreate(
            name="Oversized Document",
            metadata={"description": "This document exceeds size limits"},
            is_public=False,
            mime_type="application/octet-stream",
        )
        test_file = create_test_file(content=oversized_content)
