...
class TestUserService:
    def test_register_user_success(self, user_service: UserService):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("ab", "Username must be between 3 and 50 characters long"),
            ("a" * 51, "Username must be between 3 and 50 characters long"),
            ("123user", "Username must start with a letter"),
            ("user__name", "Username cannot contain consecutive special characters"),
            ("admin", "This username is reserved"),
            ("test1234567", "Username cannot contain more than 3 consecutive numbers"),
            ("user@name", "Username must start with a letter and contain only letters"),
        ],
    )
    def test_validate_username_invalid(
        self, user_service: UserService, invalid_username: str, expected_error: str
    ):
        """Test username validation with invalid usernames"""
        with pytest.raises(HTTPException) as exc_info:
            user_service._validate_username(invalid_username)
        assert expected_error in str(exc_info.value.detail)

    def test_register_duplicate_username(self, user_service: UserService):
    ...
    def test_authenticate_user_success(self, user_service: UserService):
        ...
        authenticated_user = user_service.authenticate_user(api_key)
    ...
    def test_authenticate_user_wrong_secret(self, user_service: UserService):
    ...
    def test_get_user_by_id_success(self, user_service: UserService):
        ...
        # Register a user first
        username = "getuser"
        created_user, _ = user_service.register_user(username)
