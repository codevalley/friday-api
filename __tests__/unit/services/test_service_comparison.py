"""Tests to verify parity between OpenAIService - InstructorService."""
import pytest
...
def assert_processing_results_equal(
    result1: RoboProcessingResult,
    result2: RoboProcessingResult,
):
    ...
    assert (
        result1.tokens_used == result2.tokens_used
...
def mock_robo_service():
    ...
    def create_result(
        ...
        if (
            context
            and context.get("type") == "task_enrichment"
        ):
...
def test_service_parity(
    test_case: Dict[str, Any], mock_robo_service
):
...
def test_task_priority_handling(mock_robo_service):
    ...
    result = mock_robo_service.process_text(
        input_text, context={"type": "task_enrichment"}
    )

    assert "suggested_priority" in result.metadata
    assert isinstance(
        result.metadata["suggested_priority"], str
    )
    assert result.metadata[
        "suggested_priority"
    ].lower() in ["high", "medium", "low"]
...
def test_error_handling(mock_robo_service):
    ...
    with pytest.raises(Exception):
        mock_robo_service.process_text(
