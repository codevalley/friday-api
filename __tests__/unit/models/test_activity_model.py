...
def test_activity_model_initialization(sample_activity):
    ...
    assert activity.description == sample_activity["description"]
    assert activity.activity_schema == sample_activity["activity_schema"]
    assert activity.icon == sample_activity["icon"]
    assert activity.color == sample_activity["color"]
    assert activity.moments == []

def test_activity_model_db_persistence(db_session, sample_activity):
    """Test that an activity can be persisted to the database"""
    activity = Activity(**sample_activity)
    db_session.add(activity)
    db_session.commit()
    
    saved_activity = db_session.query(Activity).filter_by(name=sample_activity["name"]).first()
    assert saved_activity is not None
    assert saved_activity.name == sample_activity["name"]

def test_activity_moment_relationship(db_session, sample_activity, sample_moment):
...
def test_activity_name_unique_constraint(db_session, sample_activity):
