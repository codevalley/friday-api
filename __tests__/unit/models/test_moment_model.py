...
def test_moment_model_initialization(db_session, sample_activity, sample_moment):
...
def test_moment_model_db_persistence(db_session, sample_activity, sample_moment):
    ...
    saved_moment = db_session.query(Moment).filter_by(activity_id=activity.id).first()
    assert saved_moment is not None
    assert saved_moment.data == sample_moment["data"]

def test_moment_activity_relationship(db_session, sample_activity, sample_moment):
...
def test_moment_cascade_delete(db_session, sample_activity, sample_moment):
    ...
    assert db_session.query(Moment).filter_by(id=moment.id).first() is None
...
def test_moment_data_validation(db_session, sample_activity):
