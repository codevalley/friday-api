...
def test_create_with_instance(activity_repository, sample_activity_data):
    ...
    assert created.user_id == sample_activity_data["user_id"]


def test_create_with_fields(activity_repository, sample_activity_data):
    ...
    created = activity_repository.create(
        ...
        activity_schema=sample_activity_data["activity_schema"],
...
def test_create_invalid_color(activity_repository, sample_activity_data):
...
def test_create_invalid_schema(activity_repository, sample_activity_data):
...
def test_get_by_user_existing(activity_repository, sample_activity_data):
...
def test_get_by_user_wrong_user(activity_repository, sample_activity_data):
    """Test getting activity by ID with wrong user"""
    created = activity_repository.create(
        sample_activity_data["name"],
        user_id=sample_activity_data["user_id"],
    )
    retrieved = activity_repository.get_by_user(created.id, "wrong-user")
    assert retrieved is None


def test_get_by_user_nonexistent(activity_repository):
    """Test getting non-existent activity by ID"""
    retrieved = activity_repository.get_by_user(999, "test-user")
    assert retrieved is None


def test_get_by_name_existing(activity_repository, sample_activity_data):
...
def test_get_by_name_wrong_user(activity_repository, sample_activity_data):
...
def test_list_activities(activity_repository, sample_activity_data, test_db_session):
    ...
    activity_repository.create("Other Activity", user_id=other_user.id)
    ...
    for activity in activities:
        assert activity.user_id == sample_activity_data["user_id"]
...
def test_update_activity_success(activity_repository, sample_activity_data):
...
def test_update_activity_wrong_user(activity_repository, sample_activity_data):
...
def test_update_activity_invalid_color(activity_repository, sample_activity_data):
...
def test_delete_activity_success(activity_repository, sample_activity_data):
    ...
    assert activity_repository.get_by_user(
        created.id,
        sample_activity_data["user_id"],
    ) is None


def test_delete_activity_wrong_user(activity_repository, sample_activity_data):
    """Test deleting activity with wrong user"""
    created = activity_repository.create(
        sample_activity_data["name"],
        user_id=sample_activity_data["user_id"],
    )
    result = activity_repository.delete_activity(created.id, "wrong-user")
...
def test_delete_activity_nonexistent(activity_repository):
    ...
    result = activity_repository.delete_activity(999, "test-user")
    assert result is False


def test_validate_existence_success(activity_repository, sample_activity_data):
...
def test_validate_existence_wrong_user(activity_repository, sample_activity_data):
    ...
    with pytest.raises(HTTPException) as exc_info:
        activity_repository.validate_existence(created.id, "wrong-user")
    assert exc_info.value.status_code == 404
    assert "Activity not found or access denied" in str(exc_info.value.detail)


def test_validate_existence_nonexistent(activity_repository):
    """Test validating existence of non-existent activity"""
    with pytest.raises(HTTPException) as exc_info:
        activity_repository.validate_existence(999, "test-user")
    assert exc_info.value.status_code == 404
