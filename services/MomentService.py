...
class MomentService:
    ...
    def get_moment(self, moment_id: int) -> Optional[MomentType]:
        """Get a moment by ID"""
        moment = self.moment_repository.validate_existence(moment_id)
        return MomentType.from_db(moment) if moment else None
    ...
    def update_moment(self, moment_id: int, moment_data: MomentUpdate) -> Optional[MomentType]:
        """Update a moment"""
        moment = self.moment_repository.validate_existence(moment_id)
        activity = self.activity_repository.validate_existence(moment.activity_id)
