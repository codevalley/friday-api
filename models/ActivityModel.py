...
class Activity(EntityMeta):
    ...
    activity_schema = Column(JSON, nullable=False)  # JSON Schema for validating moment data
    icon = Column(String(255), nullable=False)  # Icon identifier or URL
    color = Column(String(50), nullable=False)  # Color code for UI representation
    ...
    __table_args__ = (
        ...
        CheckConstraint("activity_schema IS NOT NULL", name="check_schema_not_null"),
        CheckConstraint("icon IS NOT NULL AND icon != ''", name="check_icon_not_empty"),
        CheckConstraint("color IS NOT NULL AND color != ''", name="check_color_not_empty"),
    )
    ...
    def validate_schema(self):
        ...
        try:
            validate_json_schema(self.activity_schema, meta_schema)
