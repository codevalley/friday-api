from sqlalchemy import Column, Integer, DateTime, JSON, ForeignKey, CheckConstraint
...
class Moment(EntityMeta):
    ...
    data = Column(JSON, nullable=False)  # Flexible schema based on activity type
    ...
    # Constraints
    __table_args__ = (
        CheckConstraint("timestamp IS NOT NULL", name="check_timestamp_not_null"),
        CheckConstraint("activity_id IS NOT NULL", name="check_activity_id_not_null"),
        CheckConstraint("data IS NOT NULL", name="check_data_not_null"),
    )
    ...
    def validate_data(self):
        ...
        if not self.activity or not self.activity.activity_schema:
            raise ValueError("moment must be associated with an activity that has a valid schema")
        
        try:
            validate_json_schema(self.data, self.activity.activity_schema)
