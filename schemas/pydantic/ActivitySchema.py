...
class ActivityBase(BaseModel):
    ...
    activity_schema: Dict = Field(..., description="JSON Schema for validating moment data")
    ...
    def validate_color(cls, v):
        ...
        if not re.match(color_pattern, v):
            raise ValueError('Invalid color format. Use hex (#RRGGBB), rgb(r,g,b), or color name')
        return v
    ...
    def validate_schema(cls, v):
        ...
        if not all(field in v for field in required_fields):
            raise ValueError('activity_schema must be a valid JSON Schema with "type" and "properties" fields')
        return v
